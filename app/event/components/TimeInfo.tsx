...
const TimeInfo = (props:MyProps) => {
  ...
  const endTime = isNaN(+eventInfo.originalEnd) ? eventInfo.originalEnd : parseTime(`${new Date(+eventInfo.originalEnd).getHours()}:${new Date(eventInfo.originalEnd).getMinutes()}`);
  ...
  return (
    <div style={{border: '4px solid white', width: '56.5%'}}>
      <div style={{fontWeight: 'bold', fontSize: '1.3rem', textAlign: 'left', marginLeft: '10px', marginBottom: '-5px'}}>{currentEvent.split(' ').slice(0, 4).join(' ')}</div>
      <div style={{marginLeft: '1px'}}>
        <div style={{marginLeft: '-10px', paddingTop: '10px', fontWeight: 'bold', marginTop: '4px', float: 'right', width: '75%'}}>
          <div style={{fontWeight: 'bold', fontSize: '1rem'}}>{startTime} - {endTime}</div>
          <div style={{fontWeight: 'bold', fontSize: '1rem'}}>{timeZone}</div>
        </div>
        <div onClick={addToCalendar}style={{cursor: 'pointer', width: '20%', height: '20%', fill: 'white', marginLeft: '10px', marginBottom: '5px'}}>
